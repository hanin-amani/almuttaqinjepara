...
import { Send, User, MessageCircle, Loader2, Reply, X, Github } from "lucide-react";

// Struktur data komentar untuk TypeScript yang aman
...
export default function CommentSection({ infoId }: { infoId: string }) {
  ...
  // Auto-fill form data jika user sudah login lewat Next-Auth
  useEffect(() => {
    if (session?.user) {
  ...
  // Fungsi mengambil daftar komentar dari API Route internal
  ...
  // Handler kirim komentar (Tanpa Moderasi -> Langsung tampil)
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setContent("");
        setReplyTo(null);
        // Langsung panggil ulang data agar komentar baru seketika muncul di layar
        await fetchComments();
      } else {
        alert("Gagal mengirim komentar. Silakan periksa koneksi atau database Anda.");
      }
    } catch (error) {
  ...
  return (
    <div className="mt-12 space-y-8 border-t border-slate-100 pt-10 text-left">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {/* Opsi Login Cepat via Google & GitHub */}
        {!session && status !== "loading" && (
          <div className="flex items-center gap-2">
            ...
            <button 
              ...
              className="w-9 h-9 flex items-center justify-center bg-white border border-slate-200 rounded-[4px] hover:border-emerald-500 transition-all shadow-sm"
            ...
            <button 
              ...
              className="w-9 h-9 flex items-center justify-center bg-white border border-slate-200 rounded-[4px] hover:border-slate-900 transition-all text-slate-800 shadow-sm"
      ...
      <div ref={formRef} className="scroll-mt-32">
        <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 rounded-[4px] border border-slate-100 shadow-sm">
          ...
          {/* Form Biodata Tambahan jika user tidak login */}
          {!session && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-bold tracking-widest text-slate-400 uppercase">Nama Lengkap *</label>
                <input type="text" required value={name} onChange={(e) => setName(e.target.value)} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-[4px] text-sm focus:border-emerald-500 outline-none font-bold text-slate-800" />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-bold tracking-widest text-slate-400 uppercase">Email *</label>
                <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-[4px] text-sm focus:border-emerald-500 outline-none font-bold text-slate-800" />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-bold tracking-widest text-slate-400 uppercase">Website</label>
                <input type="url" value={website} onChange={(e) => setWebsite(e.target.value)} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-[4px] text-sm focus:border-emerald-500 outline-none font-bold text-slate-800" />
              </div>
            </div>
          )}
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              required
              className="w-full p-5 bg-slate-50 border border-slate-200 rounded-[4px] text-sm focus:border-emerald-500 outline-none min-h-[120px] font-medium leading-relaxed transition-all text-slate-800"
            />
          </div>
                ...
                {comments.filter(sub => sub.parent_id === c.id).map(sub => (
                  <div key={sub.id} className="flex gap-4 p-5 bg-slate-50 border border-slate-100 rounded-[4px] animate-in slide-in-from-left-2 duration-500">
                    ...
                    <div className="w-10 h-10 rounded-full overflow-hidden bg-white border border-slate-100 shrink-0 relative shadow-sm">
                      {sub.image ? (
                        <Image src={sub.image} alt={sub.name} fill className="object-cover" sizes="40px" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center text-slate-300">
                          <User size={18} />
                        </div>
