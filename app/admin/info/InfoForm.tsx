  ...
  return (
    <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-emerald-100 mb-12">
      ...
      <form action={addInfo} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-6">
            <div>
              ...
              <input name="title" required className="w-full px-5 py-4 rounded-2xl border border-gray-100 bg-gray-50 focus:bg-white focus:ring-4 focus:ring-emerald-100 outline-none font-bold" />
            </div>
            <div>
              <label className="block text-[10px] font-black text-gray-400 uppercase mb-2 tracking-widest text-left">URL Thumbnail</label>
              <input name="thumbnail" value={imageUrl} onChange={(e) => setImageUrl(e.target.value)} className="w-full px-5 py-4 rounded-2xl border border-gray-100 bg-gray-50 outline-none font-bold" placeholder="Paste link gambar di sini..." />
            </div>
            <div>
        ...
        <button type="submit" className="w-full py-5 bg-emerald-600 text-white rounded-[2rem] font-black text-sm uppercase tracking-[0.2em] hover:bg-emerald-700 transition-all">
          TERBITKAN SEKARANG 🚀
        </button>
      </form>
    </div>
