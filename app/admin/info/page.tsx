...
import { Pencil, Trash2, Loader2, Plus, MessageSquareDashed, AlertCircle, ExternalLink } from "lucide-react";
...
export default function InfoPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      // ✅ Tambahkan timestamp agar tidak kena cache agresif Next.js 16
  ...
  // ✅ LOADING STATE (Savage Pulse)
  if (loading) return (
    <div className="min-h-[400px] flex flex-col items-center justify-center space-y-4">
      <Loader2 className="animate-spin text-emerald-500" size={40} />
      <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 italic">Mensinkronkan Database RSM...</p>
    </div>
  );
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto text-left">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 bg-slate-900 p-8 rounded-[4px] shadow-2xl border-b-4 border-emerald-500 relative overflow-hidden">
        ...
        <Link href="/admin/info/create" className="relative z-10 bg-emerald-600 hover:bg-white hover:text-emerald-600 text-white px-8 py-4 rounded-[4px] text-[11px] font-black uppercase tracking-[0.2em] transition-all flex items-center gap-3 shadow-xl active:scale-95">
      ...
      {error && (
        <div className="mb-6 p-4 bg-red-50 border-l-4 border-red-500 text-red-700 flex items-center gap-3 rounded-[4px]">
          <AlertCircle size={20} />
          <p className="text-xs font-bold uppercase tracking-wide">Error: {error}</p>
        </div>
      )}

      {/* ✅ EMPTY STATE (Jika Berita Tidak Ada) */}
      {!loading && infos.length === 0 ? (
        <div className="bg-white border-2 border-dashed border-slate-100 rounded-[4px] py-32 flex flex-col items-center justify-center text-center">
          ...
          <h3 className="text-slate-900 font-black uppercase italic tracking-tight text-lg">Belum Ada Warta</h3>
          <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest mt-2 max-w-xs leading-relaxed">
            Daftar berita masih kosong atau sedang bermasalah. Coba tambahkan berita baru atau cek koneksi Supabase.
          </p>
          <button onClick={fetchData} className="mt-8 text-emerald-600 font-black text-[10px] uppercase tracking-widest hover:underline">
            Refresh Data ↻
          </button>
        </div>
      ) : (
        ...
        <div className="bg-white border border-slate-100 rounded-[4px] overflow-hidden shadow-2xl">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {infos.map((info) => (
                  <tr key={info.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-all group">
                    <td className="p-6">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                                <Link href={`/warta/${info.slug}`} target="_blank" className="text-[9px] text-slate-400 italic hover:text-blue-500 flex items-center gap-1">
                    ...
                    <td className="p-6">
                      <span className={`px-3 py-1 text-[9px] font-black uppercase rounded-full border ${
                        (info.status === 'publish' || info.status === 'published') 
                        ? 'bg-emerald-50 text-emerald-700 border-emerald-100' 
                        : 'bg-amber-50 text-amber-700 border-amber-100'
                      }`}>
                        {info.status === 'publish' || info.status === 'published' ? '🟢 LIVE' : '🟡 DRAFT'}
                      </span>
                    </td>
                    <td className="p-6 text-right">
                      <div className="flex justify-end gap-3">
                        <Link href={`/admin/info/${info.id}/edit`} className="p-3 bg-slate-900 text-emerald-400 rounded-[4px] hover:bg-emerald-600 hover:text-white transition-all shadow-lg active:scale-90">
                          <Pencil size={14} />
                        </Link>
                        <button 
...
// Icon FileText untuk background dekorasi
