...
import { Mic2, Facebook, ChevronLeft } from "lucide-react";

export default function LoginPage() {
  return (
    <main className="min-h-[90vh] flex items-center justify-center bg-slate-50 p-6">
      <div className="w-full max-w-md bg-white border border-slate-100 rounded-[4px] shadow-[0_25px_70px_rgba(0,0,0,0.07)] p-12 text-center relative overflow-hidden">
        ...
        <div className="space-y-4">
          ...
          <button 
            ...
            className="w-full flex items-center justify-center gap-4 bg-white border border-slate-200 p-4 rounded-[4px] hover:border-emerald-500 hover:bg-slate-50 transition-all group active:scale-95 shadow-sm"
          >
            ...
            <span className="text-[11px] font-black uppercase tracking-widest text-slate-700">Masuk dengan Google</span>
          </button>

          {/* ✅ FACEBOOK BRAND */}
          <button 
            onClick={() => signIn("facebook", { callbackUrl: "/" })}
            className="w-full flex items-center justify-center gap-4 bg-[#1877F2] text-white p-4 rounded-[4px] hover:bg-[#166fe5] transition-all shadow-md active:scale-95 border-b-4 border-[#145dbf]"
          >
            <Facebook size={20} fill="currentColor" />
            <span className="text-[11px] font-black uppercase tracking-widest italic">Masuk dengan Facebook</span>
          </button>
        </div>
